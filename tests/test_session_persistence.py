...
async def test_login_once_use_multiple_times():
    ...
    if login_result.is_error:
        print(f"Login failed (expected for test): {login_result.response_text}")
        pytest.skip("Login failed - skipping session test")
    
    print(f"Login successful: {login_result.structured_content.get('user')}")
    ...
    print(f"First call result: {first_result.is_error is False}")
    ...
    print(f"Second call result: {second_result.is_error is False    }")
    
    # Both should succeed if session is maintained
    assert first_result.is_error is False and second_result.is_error is False
...
async def test_different_tool_instances_share_session():
    ...
    print(f"Flight call: {flight_result.is_error is False}")
    ...
    print(f"Hotel call: {hotel_result.is_error is False}")
    ...
    print(f"Train call: {train_result.is_error is False }")
    
    # All should have same success status (all use same session)
    assert flight_result.is_error is False and hotel_result.is_error is False and train_result.is_error is False
...
async def test_new_factory_requires_new_login():
    ...
    print(f"Factory 1 booking result: {result1.is_error is False}")
    ...
    print(f"Factory 2 booking result: {result2.is_error is False}")
    ...
    if result2.is_error is False:
        print("✅ Factory is singleton - session persisted across get_tool_factory() calls")
    else:
        assert "USER_NOT_LOGGED_IN" in result2.response_text
...
async def test_bookings_without_login_should_fail():
    ...
    assert flight_result.is_error is True
    assert hotel_result.is_error is True
    assert train_result.is_error is True

    assert "USER_NOT_LOGGED_IN" in flight_result.response_text
    assert "USER_NOT_LOGGED_IN" in hotel_result.response_text
    assert "USER_NOT_LOGGED_IN" in train_result.response_text
...
async def test_session_persists_across_hotel_search():
    ...
    print(f"First booking call: {first_booking_result.is_error is False}")
    ...
    print(f"Second booking call: {second_booking_result.is_error is False}")
    
    # Both booking calls should have same success status
    assert first_booking_result.is_error is False
    assert second_booking_result.is_error is False  
