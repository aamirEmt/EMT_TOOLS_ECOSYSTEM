...
<div class="flight-bookings" data-instance-id="flight-bookings-{{ instance_id }}">
  ...
  <div class="bkng-cards">
    ...
    {% for tab in tabs %}
    {% if tab.count > 3 %}
    <button class="show-more-btn" data-for-status="{{ tab.key }}" data-total="{{ tab.count }}" data-expanded="false" {% if not tab.active %}style="display:none"{% endif %} onclick="var p=this.closest('.flight-bookings');var s=this.dataset.forStatus;var t=parseInt(this.dataset.total);if(this.dataset.expanded==='true'){var idx=0;p.querySelectorAll('.bkng-card').forEach(function(c){if(c.dataset.status===s){if(idx>=3)c.style.display='none';idx++}});this.dataset.expanded='false';this.textContent='Show '+(t-3)+' More'}else{p.querySelectorAll('.bkng-card').forEach(function(c){if(c.dataset.status===s)c.style.display=''});this.dataset.expanded='true';this.textContent='Show Less'}">
