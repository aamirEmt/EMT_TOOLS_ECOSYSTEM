...
<div class="flight-bookings" data-instance-id="flight-bookings-{{ instance_id }}">
  ...
  <div class="bkng-cards">
    ...
    {% for tab in tabs %}
    {% if tab.count > 3 %}
    <button class="show-more-btn" data-for-status="{{ tab.key }}" data-total="{{ tab.count }}" {% if not tab.active %}style="display:none"{% endif %} onclick="var p=this.closest('.flight-bookings');var s=this.dataset.forStatus;p.querySelectorAll('.bkng-card').forEach(function(c){if(c.dataset.status===s)c.style.display=''});this.style.display='none'">
