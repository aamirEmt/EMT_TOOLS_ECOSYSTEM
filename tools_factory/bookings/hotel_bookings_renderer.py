...
<div class="hotel-bookings" data-instance-id="hotel-bookings-{{ instance_id }}">
  ...
  <div class="bkng-cards">
    ...
    {% for tab in tabs %}
    {% if tab.count > 3 %}
    <button class="show-more-btn" data-for-status="{{ tab.key }}" data-total="{{ tab.count }}" {% if not tab.active %}style="display:none"{% endif %} onclick="var p=this.closest('.hotel-bookings');p.querySelectorAll('.bkng-card[data-status=\''+this.dataset.forStatus+'\']').forEach(function(c){c.style.display=''});this.style.display='none'">
