      ...
      <div class="timeline-stop">
        <div class="timeline-track">
          <div class="timeline-dot {% if station.is_origin %}origin{% elif station.is_destination %}destination{% elif is_current %}current{% elif is_passed %}passed{% else %}upcoming{% endif %}">
            {% if is_passed or station.is_origin %}
...
(function() {
  ...
  function processLiveStations(stations) {
    ...
    for (var i = 0; i < total; i++) {
      ...
      result.push({
        ...
        actual_arrival: (actArr && actArr !== 'No Delay') ? actArr : null,
        actual_departure: actDep || null,
  ...
  function buildStatusHtml(data) {
    ...
    // Timeline - all stations behind Show All
    var timelineHtml = '';
    ...
    var html = '<style>' +
      ...
      '.train-status .progress-fill{height:100%;background:linear-gradient(90deg,#4caf50,#8bc34a);border-radius:3px;position:relative}' +
      ...
      '.train-status .timeline-dot.origin{background:#2e7d32;border-color:#2e7d32}' +
      '.train-status .timeline-dot.destination{background:#d32f2f;border-color:#d32f2f}' +
      '.train-status .timeline-dot.passed{background:#4caf50;border-color:#4caf50}' +
      ...
      '.train-status .timeline-line.passed{background:#4caf50}' +
